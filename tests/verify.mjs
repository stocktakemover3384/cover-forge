...
(async () => {
  ...
  // P2 · 退化输入的比例失真（仅记录，不修）
  record('F7', '退化输入行为记录（P2 已知限制）：源 < 16px 时 16px 下限会改变输出比例',
    '记录：4×4 源请求 4:3，输出被夹成 16×16（比例变为 1:1）',
    `${f2.txt} · 面板比例字段「${f2.ratio}」`,
    f2.dims && f2.dims.w === 16 && f2.dims.h === 16);
  ...
  L.push('## 残留风险 / 未覆盖场景');
  L.push('');
  L.push('- **P2（已知限制，仅记录不修）**：源图任一边小于 16px 时，输出尺寸被 16px 下限钳制，');
  L.push('  `app.js` `computeGeometry()` 的 `clamp(outW,16,12000)` 会改变宽高比。');
  L.push('  实测 4×4 源 + 4:3 → 输出 16×16（比例成 1:1），而面板比例字段仍显示「4:3 · 1.000」，');
  L.push('  属极端退化输入的观感不一致，无功能影响。');
